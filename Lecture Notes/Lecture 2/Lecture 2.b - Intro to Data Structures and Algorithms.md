...
We also sometimes see complexities like O(logN), which indicates that the complexity basically "scales with the scale of N". This is a little confusing, but it means that it is the *magnitude* of N which matters, not N itself. As a brief review which should make it clearer, log10(10) is 1, log10(100) is 2, log10(1000) is 3, and so on. Since scaling up N by some factor k takes *increasingly more* (k times as much) N for every time you do it, O(logN) grows slower than O(N). There are also complexities like O(k^N) and O(N!) which are both seriously explosive complexity. As an exercise, think about or research to see if there are any (typical) tasks which scale like this!
